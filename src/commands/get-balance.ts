...
export default class GetBalance extends Command {
  static description = 'Get specific token balance on L2'
  ...
  public async run(): Promise<void> {
    ...
    // token address
    const token = await ux.prompt('Token address on L2', { default: '0x0000000000000000000000000000000000000000'})

    // hide input while typing
    const key = await ux.prompt('Private key of the signer', { type: 'hide' })

    this.log(
      chalk.magentaBright(`Getting balance on L2 for token: ${token}`)
    );
